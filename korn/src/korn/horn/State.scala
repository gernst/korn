...
case class State(path: List[Pure], witness: List[Pure], store: Store) extends (String => Val) {
  ...
  def trace(that: Pure): State = {
    that match {
      case Pure.and(phi, psi) =>
        this and psi and phi
      case _ =>
        copy(witness = that :: witness)
    }
  }
